...
router.get('/audit', requireAuth, requireAdmin, asyncHandler(async (req: AuthenticatedRequest, res) => {
    ...
    const userId = req.query.userId ? Number.parseInt(req.query.userId as string) : undefined;
    const level = req.query.level as 'info' | 'warning' | 'error';
    const startDate = req.query.startDate ? Number.parseInt(req.query.startDate as string) : undefined;
    const endDate = req.query.endDate ? Number.parseInt(req.query.endDate as string) : undefined;
