...
import { parseJailIniFile, parseJailConfigs } from './jailConfigParser.js';
import { auditFirewallCoherence } from './FirewallAuditService.js';
...
export class Fail2banPlugin extends BasePlugin {
    ...
    getRoutes(): Router {
        ...
        // GET /firewall-audit — cohérence fail2ban ↔ iptables ↔ ipset/nftables (lecture seule, suggestions uniquement)
        router.get('/firewall-audit', auditRateLimit, requireAuth, asyncHandler(async (_req, res) => {
            if (!this.isEnabled()) throw createError('Plugin disabled', 503, 'PLUGIN_DISABLED');
            if (!this.client) return res.json({ success: true, result: { ok: false, error: 'client not initialized' } });
            const _cached = this._cachePeek<unknown>('firewall:audit', 30_000);
            if (_cached) return res.json({ success: true, result: _cached });
            const confBase    = this.resolveDockerPathSync('/etc/fail2ban');
            const jailMeta    = parseJailConfigs(confBase);
            const activeJails = await this.client.listJails();
            const result = await auditFirewallCoherence({ confBase, client: this.client, activeJails, jailMeta });
            if (result.ok) this._cachePut('firewall:audit', result);
            res.json({ success: true, result });
        }));
