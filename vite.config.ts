...
export default defineConfig({
  server: {
    ...
    hmr: {
      clientPort: parseInt(process.env.DASHBOARD_PORT || process.env.VITE_PORT || '5175', 10),
      // Vite will automatically detect the host from the browser's window.location.hostname
      // So if you access via 192.168.1.150:3777, HMR will use ws://192.168.1.150:3777
    },
    proxy: {
      '/api': {
        // Use localhost for proxy - Vite proxy runs on the same machine as the backend
        // When accessing via IP (192.168.1.150), the proxy still connects to localhost:3004
