...
const SCard: React.FC<{
    ...
    titleTooltip?: {
        ...
        color?: import('./helpers').F2bTtColor;
    };
    ...
    return (
        ...
        >
            ...
            >
                ...
                {titleTooltip ? (
                    <F2bTooltip title={title} bodyNode={titleTooltip.bodyNode} color={titleTooltip.color ?? 'blue'}>
                        {titleEl}
                    </F2bTooltip>
                ) : (
                    titleEl
...
const DomainDetailModal: React.FC<{
    ...
    useEffect(() => {
        setLoading(true);
        setData(null);
        api.get<DomainDetailData>(
            `/api/plugins/fail2ban/tops/domain-detail?domain=${encodeURIComponent(domain)}&days=${days}`,
        )
            .then((res) => {
                if (res.success && res.result?.ok) setData(res.result);
            })
                    ...
                    {!loading && data && bans.length > 0 && (
                        ...
                        >
                                    ...
                                    {[
                                        ...
                                        <th
                                            ...
                                            style={{
                                                ...
                                                letterSpacing: '.04em',
                                                color: C.muted,
                                                textAlign: h === 'IP' || h === 'Jail' ? 'left' : 'center',
                            ...
                            <tbody>
                                {bans.map((b, i) => (
                                    <tr
                                        key={`${b.ip}-${b.timeofban}`}
                                        style={{
                                            borderBottom: `1px solid ${C.border}`,
                                            background: i % 2 === 0 ? 'transparent' : `${C.bg1}55`,
                                        }}
                                    >
                                        <td
                                            style={{
...
type IpSetHist = {
    ipset_names: string[];
    ipset_days: Record<string, Record<string, number>>;
} | null;
        ...
        >
            ...
            <div ref={containerRef} style={{ width: '100%' }}>
                ...
                >
                    ...
                    <text x={3} y={yOf(maxV * 0.5) - 2} fontSize={9} fill="rgba(128,128,128,.4)">
                        {Math.round(maxV * 0.5)}
                    </text>
                    {/* Lines per set */}
                    {names.map((nm, ni) => {
                        ...
                        const lastDtIdx = [...plotDates]
                            .map((dt, i) => ({ dt, i, v: plotMap[dt]?.[nm] }))
                            .reverse()
                            .find((x) => x.v != null);
                        return (
                            <g key={nm}>
                                <polygon
                                    points={`${pts.join(' ')} ${lastX},${yOf(0)} ${firstX},${yOf(0)}`}
                                    fill={color}
                                    opacity={0.06}
                                />
                                <polyline
                                ...
                                {plotDates.map((dt, i) => {
                                    ...
                                    return (
                                        <circle key={dt} cx={xOf(i)} cy={yOf(v)} r={4} fill="transparent" stroke="none">
                                            <title>{tipText(dt, nm, v)}</title>
                                        </circle>
                                    );
                                })}
                                {lastDtIdx && (
                                    <>
                                        ...
                                        />
                                        <text
                                            x={xOf(lastDtIdx.i) + (lastDtIdx.i > n * 0.8 ? -6 : 6)}
                                            y={yOf(lastDtIdx.v!) - 4}
                                            fontSize={9}
                                            fill={color}
                                            textAnchor={lastDtIdx.i > n * 0.8 ? 'end' : 'start'}
                                            fontWeight={600}
                                        >
                                            {lastDtIdx.v}
                                        </text>
            ...
            {(() => {
                ...
                const sortedLegend = [...names].sort((a, b) => {
                    const ia = cmKeys.indexOf(a);
                    const ib = cmKeys.indexOf(b);
...
const IpSetsSection: React.FC<{
    ...
    useEffect(() => {
        const cached = getCached<{
            sets: { name: string; entries: number; type: string }[];
        }>('ipset:info');
        if (cached) {
            setSets(
                [...cached.sets].filter((s) => !s.name.startsWith('docker-')).sort((a, b) => b.entries - a.entries),
            );
            setLoading(false);
        }
        api.get<{
    ...
    const slices = sets
        ...
        .map((s, i) => {
            ...
            const start = (acc * Math.PI) / 180;
            const end = ((acc + deg) * Math.PI) / 180;
            if (!hidden) acc += deg;
            ...
            return {
                ...
                y2,
                large: deg >= 180 ? 1 : 0,
                pct: visibleTotal > 0 ? Math.round((s.entries / visibleTotal) * 100) : 0,
    ...
    return (
        ...
        >
            ...
            <HistChart
                ...
                nonEmptyNames={new Set(barSets.map((s) => s.name))}
                colorMap={colorMap}
            />
                    ...
                    >
                        ...
                        <div
                            style={{
                                flexShrink: 0,
                                display: 'grid',
                                gridTemplateColumns: `repeat(${barCols}, max-content)`,
                                gridTemplateRows: `repeat(${Math.ceil(sets.length / barCols)}, auto)`,
                                gridAutoFlow: 'column',
                                columnGap: '1.5rem',
                                rowGap: 0,
                                marginTop: '.5rem',
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            <div
                                                style={{
                                                    width: `${s.entries > 0 ? Math.max(2, (s.entries / maxEntries) * 100) : 0}%`,
                                                    height: '100%',
                                                    background: barColor,
                                                    borderRadius: 3,
                                                }}
                                    ...
                                    >
                                        <span
                                            style={{
                                                ...
                                                height: 8,
                                                borderRadius: '50%',
                                                background: s.hidden ? C.muted : s.color,
                                                flexShrink: 0,
                                            }}
                                        />
                                        <span
                                            style={{
                                                ...
                                                textOverflow: 'ellipsis',
                                                whiteSpace: 'nowrap',
                                                color: s.hidden ? C.muted : s.color,
                                            }}
                                        >
                                            {s.name}
                                        </span>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <svg viewBox="0 0 160 160" style={{ width: 130, height: 130, flexShrink: 0 }}>
                                            {slices
                                                .filter((s) => !s.hidden && s.deg > 0)
                                                .map((s) => (
                                                    <path
                                                        key={s.name}
                                                        d={`M 80 80 L ${s.x1} ${s.y1} A 65 65 0 ${s.large} 1 ${s.x2} ${s.y2} Z`}
                                                        fill={s.color}
                                                        stroke={C.bg1}
                                                        strokeWidth={1.5}
                                                        opacity={0.9}
...
const TopCard: React.FC<{
    ...
    viewMode: 'bar' | 'pie';
    limit: number;
    rowPrefix?: (label: string) => React.ReactNode;
    emptyMsg?: string;
    ...
    rowTooltip?: (
        label: string,
        entry: TopEntry,
    ) => { title: string; bodyNode: React.ReactNode; color: F2bTtColor } | null;
    titleTooltip?: {
        bodyNode: React.ReactNode;
        color?: F2bTtColor;
        width?: number;
    ...
    const visibleTotal = pieEntries
        .filter((e) => !hiddenPie.has(labelKey === 'ip' ? e.ip! : e.jail!))
    ...
    const pieSlices = pieEntries.map((e, i) => {
        ...
        return {
            label,
            count: e.count,
            pct: visibleTotal > 0 ? Math.round((e.count / visibleTotal) * 100) : 0,
            color: PIE_COLORS[i % PIE_COLORS.length],
            x1,
            y1,
            x2,
            ...
            >
                ...
                <span style={{ fontWeight: 600, fontSize: '.82rem' }}>
                    {titleTooltip ? (
                        <F2bTooltip
                            title={title}
                            bodyNode={titleTooltip.bodyNode}
                            color={titleTooltip.color ?? 'cyan'}
                            width={titleTooltip.width ?? 320}
                        >
                            <span
                                style={{
                                    cursor: 'help',
                ...
                {periodLabel && (
                    <span style={{ fontSize: '.65rem', color: C.muted, fontWeight: 400 }}>({periodLabel})</span>
                )}
                {!loading && (
                    <span
                        style={{
                ...
                {loading ? (
                            ...
                            >
                                ...
                                <div
                                    style={{
                                        ...
                                        background: C.bg3,
                                        borderRadius: 3,
                                        animation: `f2b-shimmer 1.6s ease-in-out ${(i * 0.12).toFixed(2)}s infinite`,
                                    }}
                                />
                                <div
                                    style={{
                        ...
                        <svg viewBox="0 0 140 140" style={{ width: 120, height: 120, flexShrink: 0 }}>
                            {pieSlices
                                ...
                                .map((s) => (
                                    <path
                                        key={s.label}
                                        d={`M 70 70 L ${s.x1} ${s.y1} A 58 58 0 ${s.large} 1 ${s.x2} ${s.y2} Z`}
                                        fill={s.color}
                                        stroke={C.bg1}
                                        strokeWidth={1.5}
                                        opacity={0.9}
                            ...
                            <text x={70} y={73} fontSize={10} fontWeight={700} fill={C.text} textAnchor="middle">
                                {visibleTotal}
                            </text>
                        </svg>
                        <div
                                    ...
                                    >
                                        <span
                                            style={{
                                                ...
                                                height: 8,
                                                borderRadius: '50%',
                                                background: s.hidden ? C.muted : PIE_COLORS[i % PIE_COLORS.length],
                                                flexShrink: 0,
                                                marginTop: 3,
                                            }}
                                        />
                                        {clickable ? (
                                            ...
                                            <span
                                                style={{
                                                    ...
                                                    fontFamily: 'monospace',
                                                    fontSize: '.72rem',
                                                    color: s.hidden ? C.muted : PIE_COLORS[i % PIE_COLORS.length],
                                                    wordBreak: 'break-all',
                                                    lineHeight: 1.3,
                                                }}
                                            >
                    ...
                    >
                        {displayed.map((e, i) => {
                            ...
                            const handleLabelClick = onLabelClick
                                ? () => onLabelClick(label)
                                : onIpClick
                                  ? () => onIpClick(label)
                                  : undefined;
                                    ...
                                    >
                                        <div
                                            style={{
                                                ...
                                                height: '100%',
                                                borderRadius: 2,
                                                background: `linear-gradient(to right, ${color}, ${color}${Math.round(
                                                    rankOpacity * 255,
                                                )
                                                    .toString(16)
                                                    .padStart(2, '0')})`,
                        ...
                        {hasMore && (
                            <div
                                style={{
                                    ...
                                    right: 0,
                                    height: 36,
                                    background: `linear-gradient(to bottom, transparent, ${C.bg0})`,
                                    pointerEvents: 'none',
...
const ResumePeriodeSection: React.FC<{
    ...
    return (
        <SCard
            ...
            color={C.green}
            title={t('fail2ban.stats.periodSummary')}
            sub={`(${periodLabel})`}
        ...
        >
            ...
            >
                {[
                    {
                        l: t('fail2ban.status.totalBans'),
                        k: 'totalBans' as SummaryKey | null,
                        v: data?.totalBans ?? 0,
                        c: C.red,
                        small: false,
                    ...
                    {
                        l: t('fail2ban.status.uniqueIps'),
                        k: 'uniqueIps' as SummaryKey | null,
                        v: data?.uniqueIps ?? 0,
                        c: C.blue,
                        small: false,
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 0 }}>
                            <span
                                style={{
                                    fontSize: s.small ? '1rem' : '1.6rem',
                                    fontWeight: 700,
                                    color: s.c,
                                    lineHeight: 1.1,
                                    fontFamily: s.small ? 'monospace' : undefined,
                                    overflow: 'hidden',
                                    textOverflow: 'ellipsis',
                                    whiteSpace: 'nowrap',
                                }}
...
// ── Types d'attaque ───────────────────────────────────────────────────────────
const TypesAttaqueSection: React.FC<{
    days: number;
    onDaysChange: (d: number) => void;
    data: TopsData | null;
    loading: boolean;
    ...
    const slices = jails.slice(0, 8).map((j, i) => {
        ...
        return {
            ...
            y2,
            large: deg >= 180 ? 1 : 0,
            pct: Math.round((j.count / total) * 100),
            color: PIE_COLORS[i % PIE_COLORS.length],
        };
    });
    ...
    return (
        <SCard
            icon={<Target style={{ width: 14, height: 14 }} />}
            color={C.red}
            title={t('fail2ban.stats.attackTypes')}
            sub={`(${periodLabel})`}
                    ...
                    >
                        {total > 0 && (
                            <svg viewBox="0 0 160 160" style={{ width: 160, height: 160, flexShrink: 0 }}>
                                ...
                                <circle cx={80} cy={80} r={22.75} fill={C.bg1} stroke={C.border} />
                                <text x={80} y={82} fontSize={11} fontWeight={700} fill={C.text} textAnchor="middle">
                                    {total}
                                </text>
                            ...
                            {jails.map((j, i) => (
                                ...
                                >
                                    ...
                                    >
                                        <div
                                            style={{
                                                width: `${Math.max(2, (j.count / Math.max(...jails.map((x) => x.count), 1)) * 100)}%`,
                                                height: '100%',
                                                background: PIE_COLORS[i % PIE_COLORS.length],
                                                borderRadius: 2,
                                            }}
...
const HeatmapSection: React.FC<{
    days: number;
    onDaysChange: (d: number) => void;
    dataKey: 'heatmap' | 'heatmapFailed';
    weekKey: 'heatmapWeek' | 'heatmapFailedWeek';
                            ...
                            >
                                ...
                                <span
                                    style={{
                                        fontSize: '.68rem',
                                        fontWeight: 600,
                                        color: d === dayTotals[peakDayIdx] ? color : '#e6edf3',
                                    }}
                                >
                                    {d}
                                </span>
                    ...
                    {loading ? (
                        ...
                        >
                            ...
                            <div>
                                ...
                                >
                                    <div />
                                    {[
                                        20, 35, 55, 80, 60, 45, 100, 90, 70, 55, 35, 45, 65, 55, 75, 95, 85, 65, 45, 35,
                                        55, 45, 35, 20,
                                    ].map((h, i) => (
                                        <div
                                            key={i}
                                            style={{
                                                ...
                                                animation: `f2b-shimmer 1.6s ease-in-out ${(i * 0.04).toFixed(2)}s infinite`,
                                            }}
                                        />
                                    ))}
                                </div>
                                            ...
                                            {Array.from({ length: 24 }, (_, hr) => (
                                                <div
                                                    ...
                                                    style={{
                                                        ...
                                                        background: C.bg3,
                                                        borderRadius: 3,
                                                        animation: `f2b-shimmer 1.8s ease-in-out ${((di * 24 + hr) * 0.005).toFixed(3)}s infinite`,
                                                    }}
                                                />
                                            ))}
                                        </React.Fragment>
                        ...
                        <>
                            ...
                            <div>
                                ...
                                >
                                    Pic&nbsp;: <strong style={{ color }}>{peakHIdx}h</strong>
                                    &nbsp;&nbsp;{hours[peakHIdx]} {label}
                                </div>
                            </div>
                                        ...
                                        <React.Fragment key={day}>
                                            ...
                                            {Array.from({ length: 24 }, (_, hr) => {
                                                ...
                                                const bg =
                                                    cnt === 0
                                                        ? C.bg3
                                                        : `rgba(${cellRgb},${(0.12 + ratio * 0.88).toFixed(2)})`;
                                                const bord =
                                                    cnt > 0
                                        ...
                                        <div
                                            ...
                                            style={{
                                                ...
                                                background:
                                                    v === 0
                                                        ? C.bg3
                                                        : `rgba(${cellRgb},${(0.12 + v * 0.88).toFixed(2)})`,
                                                borderRadius: 3,
                                                flexShrink: 0,
                                            }}
                                        />
...
const TopsSection: React.FC<{
    ...
    const topCards = [
        {
            ...
            title: t('fail2ban.stats.topIps'),
            color: C.red,
            entries: (data?.topIps ?? []).map((e) => ({ ...e })) as TopEntry[],
            labelKey: 'ip' as const,
        },
        {
            icon: <RotateCcw style={{ width: 12, height: 12 }} />,
            title: t('fail2ban.attackCategories.recidivist'),
            color: C.red,
            entries: (data?.topRecidivists ?? []).map((e) => ({
                ...e,
            })) as TopEntry[],
            labelKey: 'ip' as const,
        },
        {
            ...
            title: t('fail2ban.stats.topJails'),
            color: C.orange,
            entries: (data?.topJails ?? []).map((e) => ({
                ip: undefined,
                jail: e.jail,
                count: e.count,
            })) as TopEntry[],
            labelKey: 'jail' as const,
        },
    ];
    ...
    const domainsEmptyMsgBans = domainsEmptyMsg(t('fail2ban.stats.domainsEmptyBans'));
    const domainsEmptyMsgFails = domainsEmptyMsg(t('fail2ban.stats.domainsEmptyFails'));
    const domainBanEntries: TopEntry[] = (data?.topDomains ?? []).map((e) => ({
        ip: e.domain,
        count: e.count,
        secondary: e.failures ?? 0,
        files: e.files,
    }));
                        ...
                        {list.map((f) => (
                            <div
                                ...
                                style={{
                                    ...
                                    gap: '.4rem',
                                    fontSize: '.75rem',
                                    color: f.kind === 'access' ? '#e6edf3' : '#8b949e',
                                    fontFamily: 'monospace',
                                }}
                            >
                                <FileText
                                    style={{
                                        width: 11,
                                        height: 11,
                                        color: f.kind === 'access' ? '#39c5cf' : '#8b949e',
                                        flexShrink: 0,
                                    }}
                                />
                                <span>{f.name}</span>
    ...
    const domainBanTooltip = (
        label: string,
        e: TopEntry,
    ): { title: string; bodyNode: React.ReactNode; color: F2bTtColor } => ({
        title: label,
        color: 'cyan',
        bodyNode: (
    ...
    const jailRows = [...jails]
        .sort((a, b) => (b.totalBannedSqlite ?? b.totalBanned) - (a.totalBannedSqlite ?? a.totalBanned))
        .slice(0, 12);
        ...
        <SCard
            ...
            right={
                <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem' }}>
                    ...
                    {TOP_LIMITS.map((l) => (
                        <button key={l} onClick={() => setTopLimit(l)} style={btnStyle(topLimit === l)}>
                            {l === 0 ? 'Tous' : l}
                        </button>
                    ))}
                    <div style={{ width: 1, height: 14, background: C.border }} />
                    <button onClick={() => setViewMode('bar')} style={btnStyle(viewMode === 'bar')}>
                        ▐▌ Barres
                    </button>
                    <button onClick={() => setViewMode('pie')} style={btnStyle(viewMode === 'pie')}>
                        ◑ Camembert
                    </button>
                </div>
            }
        ...
        >
            ...
            >
                ...
                <TopCard
                    ...
                    title={t('fail2ban.stats.topDomainsBans')}
                    color={C.cyan}
                    periodLabel={periodLabel}
                    ...
                    rowPrefix={(domain) => (
                        <>
                            <img
                                src="/icons/services/nginx-proxy-manager.svg"
                                width={13}
                                height={13}
                ...
                <TopCard
                    icon={<TrendingUp style={{ width: 12, height: 12 }} />}
                    title={t('fail2ban.stats.topDomainsFails')}
                    color={C.orange}
                    periodLabel={periodLabel}
                ...
                {jailRows.length > 0 &&
                    (() => {
                        ...
                        const rSlices = jailRows.slice(0, 8).map((j, i) => {
                            ...
                            const start = (rAcc * Math.PI) / 180;
                            const end = ((rAcc + deg) * Math.PI) / 180;
                            rAcc += deg;
                            const [cx, cy, r] = [70, 70, 58];
                            const x1 = cx + r * Math.cos(start);
                            const y1 = cy + r * Math.sin(start);
                            ...
                            return {
                                jail: j.jail,
                                v,
                                pct: rTotal > 0 ? Math.round((v / rTotal) * 100) : 0,
                                color: PIE_COLORS[i % PIE_COLORS.length],
                                x1,
                                y1,
                                x2,
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <div
                                                            style={{
                                                                width: `${Math.max(4, (v / jailMax) * 100)}%`,
                                                                height: '100%',
                                                                background: C.purple,
                                                                borderRadius: 2,
                                                            }}
...
const WhitelistStatsSection: React.FC = () => {
        ...
        api.get<WhitelistData>('/api/plugins/fail2ban/whitelist/stats')
            .then((res) => {
                if (res.success && res.result?.ok) {
                    setCached('whitelist:stats', res.result);
                    setData(res.result);
                } else if (!cached)
                    setError(res.result?.ok === false ? t('fail2ban.status.noData') : t('fail2ban.errors.unknown'));
            })
            .catch((e) => {
                if (!cached) setError(String(e));
            })
    ...
    const toggleJail = (jail: string) =>
        setExpanded((prev) => {
            const s = new Set(prev);
            s.has(jail) ? s.delete(jail) : s.add(jail);
            return s;
        });
                ...
                >
                    ...
                    <div>
                        ...
                        >
                            ...
                            />
                            Global [DEFAULT] — {global.length} IP
                            {global.length !== 1 ? 's' : ''}
                        </div>
                        {global.length === 0 ? (
                            <span
                            ...
                            >
                                Aucune IP whitelistée globalement
                            </span>
                        ) : (
                                                ...
                                                >
                                                    {j.extra.length > 0 && (
                                                        <div>
                                                            ...
                                                            >
                                                                {j.extra.map((ip) => (
                                                                    <IpChip key={ip} ip={ip} color={C.green} />
                                                                ))}
                                                            </div>
                                                        </div>
                                                    )}
                                                    {j.missing.length > 0 && (
                                                        <div>
                                                            ...
                                                            >
                                                                Absents du global (ignorés dans ce jail) :
                                                            </div>
                                                            <div
                                                                style={{
                                                                    display: 'flex',
                                                            ...
                                                            >
                                                                {j.missing.map((ip) => (
                                                                    <IpChip key={ip} ip={ip} color={C.red} />
                                                                ))}
                                                            </div>
                                                        </div>
                                                    )}
...
interface SafeBannedData {
    ok: boolean;
    hits: SafeHit[];
    providers: Record<string, { color: string; desc: string }>;
}
...
const SafeBannedSection: React.FC<{ onIpClick?: (ip: string) => void }> = ({ onIpClick }) => {
    ...
    useEffect(() => {
        ...
        api.get<SafeBannedData>('/api/plugins/fail2ban/whitelist/safe-banned')
            .then((res) => {
                if (res.success && res.result?.ok) {
                    setCached('whitelist:safe-banned', res.result);
                    setData(res.result);
    ...
    return (
        <SCard
            ...
            title={t('fail2ban.stats.bansToCheck')}
            sub={titleSub}
            collapsible
            defaultOpen={false}
        >
            ...
            {!loading && !error && hits.length > 0 && (
                <div style={{ padding: '.5rem 0 .75rem' }}>
                    ...
                    >
                        ...
                        Ces IPs sont connues comme légitimes (proxy, DNS, monitoring, SEO) — vérifier et débannir si
                        nécessaire.
                    </div>
                        ...
                        {hits.map((h, i) => (
                            <div
                                ...
                                style={{
                                    ...
                                    gap: '.5rem',
                                    padding: '.4rem 1rem',
                                    borderBottom: i < hits.length - 1 ? `1px solid ${C.border}` : undefined,
                                    fontSize: '.78rem',
                                }}
                            >
                                <span
                                ...
                                <span
                                    style={{
                                        fontFamily: 'monospace',
                                        color: onIpClick ? C.blue : C.text,
                                        cursor: onIpClick ? 'pointer' : undefined,
                                        flexShrink: 0,
                                        minWidth: 120,
                                    }}
                                ...
                                <span style={{ fontSize: '.7rem', color: C.muted, flexShrink: 0 }}>
                                    jail: <span style={{ color: C.purple, fontFamily: 'monospace' }}>{h.jail}</span>
                                </span>
                                <span style={{ fontSize: '.7rem', color: C.orange, flexShrink: 0 }}>
                                    {fmtExpiry(h)}
                                </span>
                            </div>
                        ))}
...
const StatsSummaryBanner: React.FC<{
    ...
    const fetchFailing = useCallback(() => {
        api.get<{
            ok: boolean;
            ips: { jail: string; ip: string; count: number; lastSeen: number }[];
        }>('/api/plugins/fail2ban/failing-ips')
            .then((res) => {
                if (res.success && res.result?.ok) setFailingIps(res.result.ips);
            })
            .catch(() => {});
    }, []);
    ...
    const failuresDelta =
        summary && prevSummary && prevSummary.totalFailures > 0
            ? summary.totalFailures - prevSummary.totalFailures
            : null;
        ...
        >
            ...
            >
                ...
                <span style={{ fontSize: '.75rem', color: C.muted }}>
                    <span
                        style={{
                            color: totalBanned > 0 ? C.red : C.green,
                            fontWeight: 700,
                            fontSize: '.88rem',
                        }}
                    >
                ...
                {ipsetSetCount > 0 && (
                    <>
                        <F2bTooltip
                            title={t('fail2ban.stats.ipsetProtection')}
                            bodyNode={
                                <div
                                    style={{
                                        display: 'flex',
            ...
            {statusHydrated && (jailsUnderPressure.length > 0 || failuresTrendUp) && (
                <div
                    style={{
                        ...
                        flexWrap: 'wrap',
                        borderTop: `1px solid ${C.border}`,
                        background: failuresTrendUp ? `${C.orange}08` : undefined,
                    }}
                >
                    {/* Alerte hausse tentatives */}
                    {failuresTrendUp && failuresDelta !== null && (
                        <F2bTooltip
                            title={t('fail2ban.stats.attemptsRising')}
                            bodyNode={
                                <div
                                    style={{
                                        display: 'flex',
                        ...
                        >
                            ...
                            >
                                ...
                                {t('fail2ban.stats.attemptsRisingDelta', {
                                    delta: failuresDelta.toLocaleString('fr-FR'),
                                })}
                            </span>
                        </F2bTooltip>
                    )}
...
export const TabStats: React.FC<TabStatsProps> = ({
    ...
    const fetchTops = useCallback(() => {
        ...
        if (!fullCached) {
            api.get<TopsData>(`/api/plugins/fail2ban/tops?days=${days}&phase=fast`, {
                signal: ac.signal,
            })
                .then((res) => {
                    ...
                    if (res.success && res.result?.ok) {
                        setTopsData((prev) => {
                            ...
                            return {
                                ...
                                topRecidivists: base.topRecidivists?.length
                                    ? base.topRecidivists
                                    : res.result.topRecidivists,
                            };
                        });
        ...
        api.get<TopsData>(`/api/plugins/fail2ban/tops?days=${days}&limit=100&compare=1`, { signal: ac.signal })
            ...
            .finally(() => {
                ...
                }>(`/api/plugins/fail2ban/tops/domains?days=${days}&limit=100`, {
                    signal: ac.signal,
                })
                    .then((res) => {
                        if (ac.signal.aborted) return;
                        if (res.success && res.result) {
                            ...
                            setTopsData((prev) => {
                                ...
                                return {
                                    ...prev,
                                    topDomains: nextTopDomains,
                                    domainsWarning: nextWarning,
                                };
                            });
                        }
                    })
    ...
    useEffect(() => {
        ...
        return () => {
            clearTimeout(timer);
            prewarmAcs.forEach((ac) => ac.abort());
        };
    }, [topsData, topsRefreshing, topsLoading, days]);

    const topActiveJail =
        jails.filter((j) => j.currentlyBanned > 0).sort((a, b) => b.currentlyBanned - a.currentlyBanned)[0]?.jail ??
        null;
    ...
    const statCards: {
        ...
        {
            label: t('fail2ban.status.activeJails'),
            value: activeJails,
            icon: <Shield style={{ width: 14, height: 14 }} />,
            color: C.blue,
        ...
        {
            label: t('fail2ban.status.bansActive'),
            value: totalBanned,
            icon: <Ban style={{ width: 14, height: 14 }} />,
            color: C.red,
            tt: (
        ...
        {
            label: t('fail2ban.status.failuresCurrent'),
            value: totalFailed,
            icon: <AlertTriangle style={{ width: 14, height: 14 }} />,
            color: C.orange,
        },
        {
            label: t('fail2ban.stats.totalBansCumul'),
            value: totalAllTime,
            icon: <ShieldOff style={{ width: 14, height: 14 }} />,
            color: C.purple,
        },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            >
                ...
                <SCard
                    icon={<Shield style={{ width: 14, height: 14 }} />}
                    color={C.blue}
                    title={t('fail2ban.stats.currentState')}
                >
                    <div
                        style={{
                            padding: '1rem',
                    ...
                    >
                        ...
                        >
                            {statCards.map(({ label, value, icon, color, tt, valueSub }) => {
                                ...
                                return tt ? (
                                    <F2bTooltip key={label} title={label} bodyNode={tt} color="red" block>
                                        {inner}
                                    </F2bTooltip>
                                ) : (
                                    inner
                        ...
                        {firstEventAt && (
                            ...
                            >
                                ...
                                <span style={{ fontFamily: 'monospace', color: '#58a6ff' }}>
                                    {new Date(firstEventAt * 1000).toLocaleTimeString('fr-FR', {
                                        hour: '2-digit',
                                        minute: '2-digit',
                                    })}
                                </span>
                ...
                <ResumePeriodeSection
                    days={days}
                    onDaysChange={onDaysChange}
                    data={topsData?.summary ?? null}
                    prev={topsData?.prevSummary ?? null}
                    loading={topsLoading}
                />
            </div>
            ...
            >
                <HeatmapSection
                    dataKey="heatmap"
                    weekKey="heatmapWeek"
                    monthKey="heatmapMonth"
                    title={t('fail2ban.stats.bansByHour')}
                ...
                />
                <HeatmapSection
                    dataKey="heatmapFailed"
                    weekKey="heatmapFailedWeek"
                    monthKey="heatmapFailedMonth"
                    title={t('fail2ban.stats.attemptsByHour')}
                    icon={<AlertTriangle style={{ width: 14, height: 14 }} />}
                    color={C.orange}
                    barRgb="227,179,65"
                    cellRgb="227,179,65"
                ...
                <SCard
                    icon={<Shield style={{ width: 14, height: 14 }} />}
                    color={C.blue}
                    title={t('fail2ban.stats.jailSummary')}
                    collapsible
                >
                                    ...
                                    {[
                                        ...
                                        <th
                                            ...
                                            style={{
                                                ...
                                                letterSpacing: '.04em',
                                                color: C.muted,
                                                textAlign: h === t('fail2ban.labels.jail') ? 'left' : 'center',
                                                whiteSpace: 'nowrap' as const,
                                            }}
                                        >
                                            {h}
                            ...
                            <tbody>
                                {jails.length === 0 ? (
                                    <tr>
                                        ...
                                        >
                                            {t('fail2ban.messages.noActiveJails')}
                                        </td>
                                    </tr>
                                ) : (
                                    jails.map((j) => {
                                            ...
                                            >
                                                ...
                                                <td
                                                    style={{
                                                        padding: '.38rem .55rem',
                                                        textAlign: 'center',
                                                        color: j.bansInPeriod !== undefined ? C.cyan : C.muted,
                                                        fontSize: '.78rem',
                                                    }}
                                                >
