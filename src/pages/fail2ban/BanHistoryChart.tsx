...
export const BanHistoryChart: React.FC<BanHistoryChartProps> = ({
    ...
    const toggleJail = (jail: string) => {
        setHidden((prev) => {
            ...
            if (next.has(jail)) next.delete(jail);
            else next.add(jail);
            return next;
        });
    };
    ...
    const jailTotals = (jail: string) => Object.values(byJail[jail] ?? {}).reduce((s, v) => s + v, 0);
                    ...
                    <F2bTooltip key={jail} title={jail} body={ttBody} color={colorToTt(jailColor(jail, jailNames))}>
                        <button
                            ...
                            style={{
                                ...
                                padding: '.15rem .45rem',
                                borderRadius: 4,
                                border: `1px solid ${isHidden ? '#30363d' : color + '55'}`,
            ...
            ? (() => {
                  ...
                  if (delta > 0)
                      return (
                          ...
                          >
                              ▲ +{delta > 9999 ? `${(delta / 1000).toFixed(1)}k` : delta}
                          </span>
                      );
                  if (delta < 0)
                      return (
                          ...
                          >
                              ▼ {delta > -9999 ? delta : `-${(Math.abs(delta) / 1000).toFixed(1)}k`}
                          </span>
                      );
                  return (
                      <span
