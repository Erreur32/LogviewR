...
const NAV_GROUPS = [
    {
        ...
        items: [
            {
                id: 'jails' as TabId,
                labelKey: 'fail2ban.tabs.jails',
                icon: Shield,
                color: '#58a6ff',
            },
            {
                id: 'filtres' as TabId,
                labelKey: 'fail2ban.tabs.filters',
                icon: Filter,
                color: '#3fb950',
            },
            {
                id: 'actions' as TabId,
                labelKey: 'fail2ban.tabs.actions',
                icon: Zap,
                color: '#e3b341',
            },
        ],
    },
    {
        ...
        items: [
            {
                id: 'tracker' as TabId,
                labelKey: 'fail2ban.tabs.tracker',
                icon: List,
                color: '#e3b341',
            },
            {
                id: 'carte' as TabId,
                labelKey: 'fail2ban.tabs.map',
                icon: MapIcon,
                color: '#39c5cf',
            },
            {
                id: 'stats' as TabId,
                labelKey: 'fail2ban.tabs.stats',
                icon: Activity,
                color: '#58a6ff',
            },
        ],
    },
    {
        ...
        items: [
            {
                id: 'iptables' as TabId,
                labelKey: 'fail2ban.tabs.iptables',
                icon: Network,
                color: '#39c5cf',
            },
            {
                id: 'ipset' as TabId,
                labelKey: 'fail2ban.tabs.ipset',
                icon: Database,
                color: '#bc8cff',
            },
            {
                id: 'blocklists' as TabId,
                labelKey: 'fail2ban.tabs.blocklists',
                icon: Shield,
                color: '#e86a65',
            },
            {
                id: 'ban' as TabId,
                labelKey: 'fail2ban.tabs.banManager',
                icon: Ban,
                color: '#e86a65',
            },
        ],
    },
    {
        ...
        items: [
            {
                id: 'config' as TabId,
                labelKey: 'fail2ban.tabs.config',
                icon: Settings,
                color: '#8b949e',
            },
            {
                id: 'audit' as TabId,
                labelKey: 'fail2ban.tabs.audit',
                icon: ClipboardList,
                color: '#8b949e',
            },
            {
                id: 'backup' as TabId,
                labelKey: 'fail2ban.tabs.backup',
                icon: Archive,
                color: '#58a6ff',
            },
            {
                id: 'aide' as TabId,
                labelKey: 'fail2ban.tabs.aide',
                icon: HelpCircle,
                color: '#8b949e',
            },
        ],
...
const Sparkline: React.FC<{ data: number[]; color: string }> = ({ data, color }) => {
    ...
    return (
        <svg
            viewBox={`0 0 ${W} ${H}`}
            style={{ width: '100%', height: H, display: 'block', marginTop: '.3rem' }}
...
export const Fail2banPage: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
    ...
    const [fwOk, setFwOk] = useState<{
        iptables: boolean | null;
        ipset: boolean | null;
    }>({ iptables: null, ipset: null });
    ...
    const checkConfigWarnings = useCallback(() => {
        api.get<{
            dbInfo: { fragPct: number; integrity: string } | null;
            appDbInfo: { fragPct: number } | null;
        }>('/api/plugins/fail2ban/config/parsed')
            .then((res) => {
    ...
    useEffect(() => {
        ...
        }>('/api/plugins/fail2ban')
            .then((res) => {
                if (res.success) {
                    const s = res.result?.settings ?? {};
                    setNpmDataPath(s.npmDataPath ?? '');
                    setNpmMysqlConfigured(
                        s.npmDbType === 'mysql' && !!s.npmMysqlHost && !!s.npmMysqlUser && !!s.npmMysqlDb,
                    );
                }
            })
            .catch(() => {});
    ...
    useEffect(() => {
        ...
        }>('/api/plugins/fail2ban/blocklists/status')
            .then((res) => {
                if (res.success && res.result?.lists) setBlocklistsStatus(res.result.lists);
            })
            .catch(() => {});
        ...
        api.get<{ ok: boolean }>('/api/plugins/fail2ban/iptables')
            .then((res) =>
                setFwOk((p) => ({
                    ...p,
                    iptables: res.success && res.result?.ok === true,
                })),
            )
    ...
    const fetchStatus = useCallback(() => {
        ...
        api.get<StatusResponse>(`/api/plugins/fail2ban/status?days=${statsDays}`, {
            ...
            .then((sRes) => {
                if (sRes.success && sRes.result) {
                    ...
                    if (Object.keys(jailDomainsRef.current).length === 0) {
                        api.get<{ ok: boolean; jail_domains?: Record<string, string> }>(
                            '/api/plugins/fail2ban/jails/enrichment',
                        )
                            .then((r) => {
                                if (r.success && r.result?.jail_domains) jailDomainsRef.current = r.result.jail_domains;
                            })
                            .catch(() => {});
                    }
            ...
            .catch(() => {
                /* keep prior status on transient errors */
            })
            .finally(() => {
        ...
        api.get<{
            ...
            jailNames?: string[];
        }>(`/api/plugins/fail2ban/history?days=${statsDays}`, {
            signal: ac.signal,
        })
            .then((hRes) => {
    ...
    const doAction = async (endpoint: string, body: object, key: string) => {
        ...
        try {
            ...
            if (res.success && res.result?.ok) {
                setActionMsg({ type: 'ok', text: res.result.output || 'OK' });
                fetchStatus();
            } else
                setActionMsg({
                    type: 'err',
                    text: res.result?.error || res.error?.message || 'Erreur',
                });
        } catch (err) {
    ...
    const topActiveJail =
        [...jails].filter((j) => j.currentlyBanned > 0).sort((a, b) => b.currentlyBanned - a.currentlyBanned)[0]
            ?.jail ?? null;
    ...
    const [failedTrend, setFailedTrend] = useState<{
        val: string;
        col: string;
    } | null>(null);
    ...
    useEffect(() => {
        ...
        }>(`/api/plugins/fail2ban/tops?days=${statsDays}&compare=1&limit=1`).then((res) => {
                    ...
                    <span>
                        ...
                        {periodBans !== prevPeriodBans ? (
                            <>
                                ...
                                <strong
                                    style={{
                                        color: periodBans > prevPeriodBans ? '#e86a65' : '#3fb950',
                                    }}
                                >
                                    {periodBans}
                                </strong>
    ...
    const miniStatCards = (
        ...
        >
            {(
                [
                    ...
                    },
                    {
                        ...
                        spark: true,
                        trendVal:
                            prevPeriodSummary && periodSummary
                                ? periodSummary.totalFailures > prevPeriodSummary.totalFailures
                                    ? `▲ +${periodSummary.totalFailures - prevPeriodSummary.totalFailures}`
                                    : periodSummary.totalFailures < prevPeriodSummary.totalFailures
                                      ? `▼ ${periodSummary.totalFailures - prevPeriodSummary.totalFailures}`
                    ...
                    },
                    {
                        ...
                        color: '#e86a65',
                        spark: true,
                        trendVal: trend(uniqueIpsTotal, prevStats?.uniqueIpsTotal),
                        trendCol: trendColor(uniqueIpsTotal, prevStats?.uniqueIpsTotal, true),
                    },
                ] as {
                    label: string;
                    value: number;
            ...
            ).map(({ label, value, icon, color, spark, trendVal, trendCol, sub, valueSub }, idx) => (
                <F2bTooltip
                    key={label}
                    block
                    title={MINI_CARD_TT[idx].ttTitle}
                    body={(MINI_CARD_TT[idx] as { ttBody?: string }).ttBody}
                    bodyNode={(MINI_CARD_TT[idx] as { ttBodyNode?: React.ReactNode }).ttBodyNode}
                    color={MINI_CARD_TT[idx].ttColor}
                >
                    <div
                        style={{
                            background: '#0d1117',
                        ...
                        blocklistsStatus.map((bl) => {
                            ...
                            return (
                                ...
                                >
                                    <span
                                        style={{
                                            color: bl.enabled ? C.green : C.muted,
                                            fontSize: '.8rem',
                                        }}
                                    >
                                        {bl.enabled ? '●' : '○'}
                                    ...
                                    <span
                                        style={{
                                            color: bl.enabled ? C.blue : C.muted,
                                            fontSize: '.72rem',
                                            marginLeft: 'auto',
                                        }}
                                    >
    ...
    const allNavItems = ([] as { id: TabId; labelKey: string; color: string }[]).concat(
        ...NAV_GROUPS.map((g) => g.items as unknown as { id: TabId; labelKey: string; color: string }[]),
    );
        ...
        <div className="flex h-full overflow-hidden" style={{ background: '#0d1117', color: '#e6edf3' }}>
            ...
            >
                                    ...
                                    >
                                        <button
                                            ...
                                            style={{
                                                ...
                                                alignItems: 'center',
                                                gap: '.55rem',
                                                padding: collapsed ? '.52rem 0' : '.45rem 1rem',
                                        ...
                                        >
                                                ...
                                                (id === 'tracker' ? (
                                                    <span
                                                        ...
                                                        style={{
                                                            ...
                                                            borderRadius: 999,
                                                            padding: '.05rem .4rem',
                                                            background: 'rgba(232,106,101,.15)',
                                                            border: '1px solid rgba(232,106,101,.25)',
                                                            whiteSpace: 'nowrap',
                                                        }}
                                                    >
                                                        <span style={{ color: '#e86a65' }}>
                                                            {trackerActive ?? totalBanned}
                                                        </span>
                                                        <span
                                                            style={{
                                                                color: '#484f58',
                                            ...
                                            {collapsed && badge !== undefined && badge > 0 && (
                                                <span
                                                    title={
                                                        id === 'config'
                                                            ? `${badge} warning${badge > 1 ? 's' : ''} — voir Config`
                                                            : id === 'tracker'
                                                              ? `${totalBanned} bannis actifs · ${trackerTotal ?? uniqueIpsTotal} total historique`
                                                              : id === 'jails'
                                                                ? `${badge} jails`
                                                                : id === 'blocklists'
                                                                  ? `${badge} liste${badge > 1 ? 's' : ''} active${badge > 1 ? 's' : ''}`
                ...
                <button
                    ...
                    style={{
                        ...
                        paddingRight: collapsed ? undefined : '1rem',
                        gap: '.35rem',
            ...
            >
                ...
                >
                        ...
                        {actionMsg && (
                            <div
                                style={{
                                    ...
                                    padding: '.2rem .65rem',
                                    borderRadius: 6,
                                    background: actionMsg.type === 'ok' ? 'rgba(63,185,80,.1)' : 'rgba(232,106,101,.1)',
                                    color: actionMsg.type === 'ok' ? '#3fb950' : '#e86a65',
                                    border: `1px solid ${actionMsg.type === 'ok' ? 'rgba(63,185,80,.25)' : 'rgba(232,106,101,.25)'}`,
                                }}
                            >
                                {actionMsg.type === 'ok' ? (
                    ...
                    >
                        <span
                            title="Auto-refresh actif toutes les 30s"
                            style={{
                                display: 'inline-flex',
                                alignItems: 'center',
                                gap: '.3rem',
                        ...
                        >
                            ...
                            {refreshBusy ? (
                                ...
                                <>
                                    ...
                                    >
                                        {new Date(lastRefreshed).toLocaleTimeString('fr-FR')}
                                    </span>
                                    <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                        ({fmtAge(lastRefreshed)})
                                    </span>
                                </>
                            ) : (
                ...
                >
                    ...
                    {tab === 'jails' && (
                        <TabJails
                            ...
                            days={statsDays}
                            onUnban={(jail, ip) =>
                                doAction('/api/plugins/fail2ban/unban', { jail, ip }, `unban-${jail}-${ip}`)
                            }
                            onBan={(jail, ip) =>
                                doAction('/api/plugins/fail2ban/ban', { jail, ip }, `ban-${jail}-${ip}`)
                            }
                    ...
                    {tab === 'tracker' && (
                        <TabTracker
                            onIpClick={(ip) => setSelectedIp(ip)}
                            onTotalChange={setTrackerTotal}
                            onActiveChange={setTrackerActive}
