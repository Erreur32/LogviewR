...
- Added 203 missing `fail2ban.*` translation keys to both `fr.json` and `en.json`. Previously untranslated keys (history, stats, map, backup, ipModal, jails, aide, fbActions, iptables, pathConfig) are now available in both languages.
...
- Fixed internationalization of tooltips on the Fail2ban page: tooltips now properly display in the user's selected language instead of defaulting to French.

### For developers

- `src/pages/Fail2banPage.tsx`: All tooltip titles on the Fail2ban page now use translation keys instead of hardcoded strings.
- Added missing translation keys for fail2ban tooltips in `src/locales/en.json` and `src/locales/fr.json`.
- `fail2ban.stats.tooltips` namespace fully implemented with keys: `dailyBans`, `activeJails`, and `bannedIps`.
